...
{
    /// <summary>
    /// Health bar display for creature stats.
    /// </summary>
    public class XenobladeCreatureInfo : MonoBehaviour
    {
        ...
        {
            stats.isEnchanted = components.Any(component => component.ToString().ToLower().Contains("enchantaura"));
            stats.isShielded = components.Any(component => component.ToString().ToLower().Contains("shieldaura"));
            stats.isArmored = components.Any(component => component.ToString().ToLower().Contains("armoraura"));
            stats.isRegen = components.Any(component => component.ToString().ToLower().Contains("regen"));
            stats.isReflecting = components.Any(component => component.ToString().ToLower().Contains("reflect"));
            stats.isDebuffResistant = components.Any(component => component.ToString().ToLower().Contains("debuffResist"));
            stats.isDamageHealed = components.Any(component => component.ToString().ToLower().Contains("damageHeal"));
            stats.isDamageImmune = components.Any(component => component.ToString().ToLower().Contains("damageImmunity") || component.ToString().ToLower().Contains("barrier"));
            stats.isPhysicalArtsPlus = components.Any(component => component.ToString().ToLower().Contains("physicalArtsPlus"));
            if (stats.isArmored && !currentBuffs.Contains(monadoArmor)) currentBuffs.Add(monadoArmor); else if (!stats.isArmored) currentBuffs.Remove(monadoArmor);
            if (stats.isShielded && !currentBuffs.Contains(monadoShield)) currentBuffs.Add(monadoShield); else if (!stats.isShielded) currentBuffs.Remove(monadoShield);
            if (stats.isEnchanted && !currentBuffs.Contains(monadoEnchant)) currentBuffs.Add(monadoEnchant); else if (!stats.isEnchanted) currentBuffs.Remove(monadoEnchant);
            ...
            if (stats.isRegen && !currentBuffs.Contains(regen)) currentBuffs.Add(regen); else if (!stats.isRegen) currentBuffs.Remove(regen);
            if (stats.isReflecting && !currentBuffs.Contains(reflect)) currentBuffs.Add(reflect); else if (!stats.isReflecting) currentBuffs.Remove(reflect);
            if (stats.isDebuffResistant && !currentBuffs.Contains(debuffResist)) currentBuffs.Add(debuffResist); else if (!stats.isDebuffResistant) currentBuffs.Remove(debuffResist);
            if (stats.isDamageHealed && !currentBuffs.Contains(damageHeal)) currentBuffs.Add(damageHeal); else if (!stats.isDamageHealed) currentBuffs.Remove(damageHeal);
            if (stats.isDamageImmune && !currentBuffs.Contains(damageImmunity)) currentBuffs.Add(damageImmunity); else if (!stats.isDamageImmune) currentBuffs.Remove(damageImmunity);
            if (stats.isPhysicalArtsPlus && !currentBuffs.Contains(physicalArtsPlus)) currentBuffs.Add(physicalArtsPlus); else if (!stats.isPhysicalArtsPlus) currentBuffs.Remove(physicalArtsPlus);
        ...
        {
            stats.isParalyzed = creature.brain.isElectrocuted;
            stats.isArtsSealed = creature.mana.currentMana <= 0.5f;
            stats.isBinded = !creature.currentLocomotion.allowMove;
            stats.isPoisoned = components.Any(component => component.ToString().ToLower().Contains("poision") || component.ToString().ToLower().Contains("venom")) || (!statuses.IsNullOrEmpty() && statuses.Any(status => status.damageType == XenobladeDamageType.Poison));
            stats.isBurning = components.Any(component => component.ToString().ToLower().Contains("burn") || component.ToString().ToLower().Contains("fire") || component.ToString().ToLower().Contains("blaze")) || (!statuses.IsNullOrEmpty() && statuses.Any(status => status.damageType == XenobladeDamageType.Blaze));
            stats.isBleeding = components.Any(component => component.ToString().ToLower().Contains("bleed") || component.ToString().ToLower().Contains("eateraura")) || (!statuses.IsNullOrEmpty() && statuses.Any(status => status.damageType == XenobladeDamageType.Bleed));
            stats.isFreezing = components.Any(component => component.ToString().ToLower().Contains("freeze") || component.ToString().ToLower().Contains("frost") || component.ToString().ToLower().Contains("chill") || component.ToString().ToLower().Contains("frozen")) || (!statuses.IsNullOrEmpty() && statuses.Any(status => status.damageType == XenobladeDamageType.Chill));
            stats.isLockedOn = components.Any(component => component.ToString().ToLower().Contains("lockon"));
            stats.isConfused = components.Any(component => component.ToString().ToLower().Contains("confuse") || component.ToString().ToLower().Contains("confusion"));
            stats.isPierced = components.Any(component => component.ToString().ToLower().Contains("pierce"));
            if (stats.isParalyzed && !currentDebuffs.Contains(paralyze)) currentDebuffs.Add(paralyze); else if (!stats.isParalyzed) currentDebuffs.Remove(paralyze);
            if (stats.isArtsSealed && !currentDebuffs.Contains(artsSeal)) currentDebuffs.Add(artsSeal); else if (!stats.isArtsSealed) currentDebuffs.Remove(artsSeal);
            if (stats.isBinded && !currentDebuffs.Contains(bind)) currentDebuffs.Add(bind); else if (!stats.isBinded) currentDebuffs.Remove(bind);
            if (stats.isPoisoned && !currentDebuffs.Contains(poison)) currentDebuffs.Add(poison); else if (!stats.isPoisoned) currentDebuffs.Remove(poison);
            if (stats.isBurning && !currentDebuffs.Contains(burn)) currentDebuffs.Add(burn); else if (!stats.isBurning) currentDebuffs.Remove(burn);
            if (stats.isBleeding && !currentDebuffs.Contains(bleed)) currentDebuffs.Add(bleed); else if (!stats.isBleeding) currentDebuffs.Remove(bleed);
            ...
            if (stats.isFreezing && !currentDebuffs.Contains(freeze)) currentDebuffs.Add(freeze); else if (!stats.isFreezing) currentDebuffs.Remove(freeze);
            if (stats.isLockedOn && !currentDebuffs.Contains(lockOn)) currentDebuffs.Add(lockOn); else if (!stats.isLockedOn) currentDebuffs.Remove(lockOn);
            if (stats.isConfused && !currentDebuffs.Contains(confuse)) currentDebuffs.Add(confuse); else if (!stats.isConfused) currentDebuffs.Remove(confuse);
            if (stats.isPierced && !currentDebuffs.Contains(pierce)) currentDebuffs.Add(pierce); else if (!stats.isPierced) currentDebuffs.Remove(pierce);
